...
export class CalendarSimulatedChartComponent implements OnInit {
    ...
    private createSvg(): void {
        d3.select('#stimulatedCalendarSVG').remove();
        this.svg = d3
            .select('#calendarSimulatedChart')
            .append('svg').attr("id","stimulatedCalendarSVG")
            // Wrapper
    ...
    private drawBars(data: any[] , others:any[]): void {
        ...
        else if(this.stimulatedDropdownFilter == 'lift'){
            key = 'lift'
            label = 'Lift %'
        }
        ...
        function mouseover(datum: any, index: any, nodes: any) {
            ...
            if (index.seasonality === 'high' || index.seasonality === 'med' || index.seasonality === 'low') {
                ...
                d3.select('#calendarSimulated-tooltip').select('#si').attr('class', 'hidden');
            }
